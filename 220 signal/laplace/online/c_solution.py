...
class SmartIrrigation:
    ...
    def A_u(self, u):
        error = -0.02 * self.t + 0.08 * np.sin(8 * self.t)
        return u + error
    
    def calculate_ise(self, h):
        h_ss = self.steady_state(h)
        error_sq = (h_ss - h)**2
        return np.trapezoid(error_sq, dx=self.dt)

    def calculate_energy(self, h):
        h_sq = h**2
        return np.trapezoid(h_sq, dx=self.dt)
...
for cfg in configs:
    ...
    for idx, (name, u) in enumerate(inputs.items()):
        ...
        U_s_cor = np.array([system.laplace_transform(u_corrupt, s) for s in s_list])
        H_s_cor = np.array([system.H_s(s, U_s) for s, U_s in zip(s_list, u_corrupt)])
        h_corr = system.inverse_laplace(s_list, H_s_cor)

        ise_clean = system.calculate_ise(h_laplace)
        e_clean = system.calculate_energy(h_laplace)
        ise_cor = system.calculate_ise(h_corr)
        e_cor = system.calculate_energy(h_corr)
        
        print(f"\n{'='*30}")
        print(f"RESULTS FOR: {name}")
        print(f"{'Metric':<15} | {'Clean':<10} | {'Corrupted':<10}")
        print(f"{'-'*40}")
        print(f"{'ISE':<15} | {ise_clean:<10.4f} | {ise_cor:<10.4f}")
        print(f"{'Energy (E)':<15} | {e_clean:<10.4f} | {e_cor:<10.4f}")
        ...
        axes[1].plot(system.t, h_corr, color="tomato", lw=2.2, label="Output h(t)")
        axes[1].set_title("corrupt Method Simulation", fontweight="bold")
